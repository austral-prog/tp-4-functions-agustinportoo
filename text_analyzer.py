...
def total_letters(text):
    ...
    """
    t_letters= count_vowels(text) + count_consonants(text)
    return t_letters
...
def vowel_percentage(text):
    ...
    """
    if not isinstance(text,(int, float)) and total_letters(text) > 0:
        p_vocales = (count_vowels(text) / total_letters(text) )* 100
        return round(p_vocales,1)
    else:
        return 0.0
